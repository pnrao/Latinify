...
Latinify is a Chrome extension (Manifest V3) that transliterates Indic scripts to the Latin script using ITRANS notation, running entirely locally in the browser. Supported scripts: Devanagari, Gujarati, Kannada, Malayalam, Odia, Telugu.
...
**Settings keys** (stored in `chrome.storage.sync`): `devanagari`, `gujarati`, `kannada`, `malayalam`, `odia`, `telugu` (booleans, default `true`), `indicateScript` (boolean, default `true`), `showStats` (boolean, default `false`).
